...
// =====================
async function extractTextFromPdfWithLib(data: ArrayBuffer): Promise<string> {
  try {
...
async function pdfToWord(file: File): Promise<Uint8Array> {
  console.log("Extracting text from PDF for Word conversion");
  ...
  try {
    extractedText = await extractTextFromPdfWithLib(arrayBuffer);
...
This may be because:
- The PDF contains scanned images (requires OCR)
- The PDF is encrypted or protected
- The text is embedded as graphics

For scanned documents, please use an OCR tool.`;
    } catch {
...
async function pdfToExcel(file: File): Promise<Uint8Array> {
  console.log("Extracting data from PDF for Excel conversion");
  ...
  try {
    const extractedText = await extractTextFromPdfWithLib(arrayBuffer);
    ...
    console.log(`Generated CSV with ${lines.length} rows`);
  } catch (err) {
    ...
    try {
      ...
      csvContent += `"Note","Text extraction failed - PDF may be image-based"\n`;
    } catch {
...
async function extractTextFromDocx(data: Uint8Array): Promise<string> {
  ...
  while ((match = paragraphRegex.exec(content)) !== null) {
    ...
    let innerMatch;
    while ((innerMatch = wtRegex.exec(paraContent)) !== null) {
  ...
  // If paragraph parsing didn't work, try simple w:t extraction
  const simpleWtRegex = /<w:t[^>]*>([^<]*)<\/w:t>/g;
  const simpleTexts: string[] = [];
  while ((match = simpleWtRegex.exec(content)) !== null) {
    const text = match[1]
      .replace(/&amp;/g, "&")
      .replace(/&lt;/g, "<")
      .replace(/&gt;/g, ">")
      .replace(/&quot;/g, '"')
      .replace(/&apos;/g, "'");
    if (text.trim()) simpleTexts.push(text);
  }
